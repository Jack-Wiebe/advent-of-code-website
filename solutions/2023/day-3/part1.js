...
module.exports = {
  run: function () {
    ...
    //input = fs.readFileSync("input", "utf-8").split(/\r\n|\r|\n/);
    console.log(input);
    ...
    function checkPerimeter(input, numbers) {
      let sum = 0;
      numbers.forEach((n) => {
        let match = false;
        let row = 0;
        console.log("parsing number: " + n.val);
        //check up
        row = n.x;
        for (let col = n.y - 1; col < n.y + n.val.length + 1; col++) {
          //console.log("checking: " + row + ",  " + col);
          if (row - 1 < 0) {
            //console.log("skip row");
            break;
          } else if (col > input[row - 1].length || col < 0) {
            //console.log("out of bounds");
            continue;
          } else if (isSymbol(input[row - 1][col])) {
        ...
        //check parallel
        //console.log("left " + input[n.x][n.y - 1]);
        //console.log("right " + input[n.x][n.y + n.val.length]);
        if (
          isSymbol(input[n.x][n.y - 1]) ||
          isSymbol(input[n.x][n.y + n.val.length])
        ) {
          console.log(">>>>>>>>>>>>>>>>>SIIIIIDE: " + n.val);
          match = true;
        }
        // //check down
        for (let col = n.y - 1; col < n.y + n.val.length + 1; col++) {
          ...
          if (row + 1 >= input.length) {
            //console.log("skip row");
            break;
          } else if (col < 0 || col > input[row + 1].length) {
            //console.log("out of bounds");
            continue;
          } else if (isSymbol(input[row + 1][col])) {
            match = true;
        ...
        if (match) {
          console.log(">>>>>>>>>>>>>>>>>>>adding: " + n.val);
          console.log("adding " + n.val + " to " + sum);
          sum += Number(n.val);
        }
    ...
    function findNumber(input) {
      ...
      for (let i = 0; i < input.length; i++) {
        for (let char = 0; char < input.length; char++) {
          if (isNumber(input[i][char])) {
            partNum += input[i][char];
          } else {
            if (partNum != "") {
              let n = {
                x: i,
                y: char - partNum.length,
                val: partNum,
              };
              numbers.push(n);
              partNum = "";
      ...
      numbers.forEach((n) => {
        console.log("X: " + n.x + "Y: " + n.y + "V: " + n.val);
      });
    ...
    // console.log(checkPerimeter(input, numbers));
    output = checkPerimeter(input, numbers);
    return output;
  },
};
